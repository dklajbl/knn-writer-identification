...
from .eval_osi_aggregation import _aggregate_OSI_metrics
...
def _is_asc(X):
    """ Returns True if 1D array elements are in ASCENDING order. """
    return np.all(np.diff(X) >= 0)


def _is_desc(X):
    """ Returns True if 1D array elements are in DESCENDING order. """
    return np.all(np.diff(X) <= 0)


def _calc_OSI_metrics_from_thr_decision_stats(thr_stats: OSI_ThresholdDecisionStats):
    """
    Calcualtes open-set identification metrics from per threshold stats.

    Parameters:
        thr_stats (OSI_ThresholdDecisionStats): Per-threshold statistics.

    Returns:
        OSI_Metrics: Open-set identification metrics calcualted from
                     the per-threshold statistics.
    """

    # checking, if the per-threshold statistics have correct order
