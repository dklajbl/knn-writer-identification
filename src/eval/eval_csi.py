...
from .metrics_identification import CSI_Metrics
...
def _calc_mAP(gallery_labels_ranked: np.ndarray, query_labels: np.ndarray):
    """
    Calcualtes Mean Average Precision (mAP).
    ...
    Parameters:
        gallery_labels_ranked (np.ndarray, shape=(N_queries, N_gallery_samples), dtype=int):
            Ranked gallery sample labels for each query.

        query_labels (np.ndarray, shape=(N_queries,), dtype=int):
            Target labels of queries.

    Returns:
        np.float32: Mean Average Precision (mAP)
